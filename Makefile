...
$(NAME):	${OBJS} ${OBJ_MAIN} $(LIBFT)
			...
			@$(call run_and_test,$(CC) $(CFLAGS) $(DFLAGS) -I$(INCLUDE_PATH) -I$(LIBFT_PATH)/includes -o $@ ${OBJS} ${OBJ_MAIN} $(LIBS))

$(LIBFT):
			@printf "$(COM_COLOR)Building libft...$(NO_COLOR)"
...
objs/%.o: 	$(SRCS_PATH)/%$(FILE_EXTENSION)
			@mkdir -p $(dir $@)
			@$(call display_progress_bar)
			@$(call run_and_test,$(CC) $(CFLAGS) $(DFLAGS) -c $< -o $@ -I$(INCLUDE_PATH) -I$(LIBFT_PATH)/includes)
